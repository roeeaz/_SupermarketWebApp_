...
import { fetchUserCartAsync } from '../counter/cartSlice';
...
export const updateDetailsAsync = createAsyncThunk(
    ...
    async (newDetails: any) => {
        console.log("Updating details with: ", newDetails); 
        const token: string = sessionStorage.getItem('token') || '';
        axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
        const updatedDetails = { newUsername: newDetails.newUsername, newPassword: newDetails.newPassword };
        const response = await axios.put("http://127.0.0.1:8000/update_user/", updatedDetails);
        return response.data;
    }
);
